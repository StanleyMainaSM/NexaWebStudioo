...
type IncomingRow = { id: string; call_id?: string; caller_id: string; callee_id: string; call_type: 'voice' | 'video'; status: string; direct_conversation_id?: string | null; admin_conversation_id?: string | null };
...
export default function GlobalCallListener() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const { data: existing } = await supabase.from('call_sessions').select('id,caller_id,callee_id,call_type,status,direct_conversation_id,admin_conversation_id').eq('callee_id', user.id).eq('status', 'ringing').order('created_at', { ascending: false }).limit(1);
      ...
      channel.on('broadcast', { event: 'incoming_call' }, ({ payload }) => {
        void open({ ...(payload as IncomingRow), id: (payload as IncomingRow).call_id || (payload as IncomingRow).id } as IncomingRow, user.id);
      });
