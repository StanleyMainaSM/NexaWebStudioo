      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid md:grid-cols-3 gap-10 mb-12">
          <div>
            ...
            <div className="flex gap-3">{[{ href: site.instagram_url, Icon: Instagram, label: 'Instagram' },{ href: site.facebook_url, Icon: Facebook, label: 'Facebook' },{ href: `mailto:${site.email}`, Icon: Mail, label: 'Email' },{ href: whatsapp, Icon: MessageCircle, label: 'WhatsApp' }].map(({ href, Icon, label }) => <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noreferrer" aria-label={label} className="w-10 h-10 rounded-xl glass flex items-center justify-center text-gray-400 hover:text-accent-400 hover:bg-white/5 transition-all"><Icon className="w-4 h-4" /></a>)}</div>
          </div>
          <div><h4 className="text-xs font-bold text-ink-500 uppercase tracking-widest mb-4">Quick Links</h4><ul className="space-y-3 text-sm">{[{ label: 'Services', to: '/services' },{ label: 'My Work', to: '/work' },{ label: 'About Me', to: '/about' },{ label: 'Pricing', to: '/pricing' },{ label: 'Reviews', to: '/reviews' },{ label: 'Contact', to: '/contact' }].map((l) => <li key={l.to}><Link to={l.to} className="text-gray-400 hover:text-accent-400 transition-colors font-medium">{l.label}</Link></li>)}</ul></div>
