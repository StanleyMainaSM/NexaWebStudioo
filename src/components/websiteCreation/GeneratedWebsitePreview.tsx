...
export default function GeneratedWebsitePreview({ specification }: { specification: WebsiteSpecification }) {
    ...
    <div className="min-h-screen bg-slate-200">
      <div className="sticky top-0 z-20 border-b border-black/10 bg-white/95 px-4 py-3 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.18em] text-slate-500">Generated website</p>
          ...
          <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1" aria-label="Preview viewport">
            ...
            <button type="button" onClick={() => setMode('mobile')} aria-label="Mobile preview" aria-pressed={mode === 'mobile'} className={`rounded-lg p-2 ${mode === 'mobile' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}><Smartphone className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
