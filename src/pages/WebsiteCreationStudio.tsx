...
export default function WebsiteCreationStudio({ creationProjectId, leadId }: { creationProjectId?: string; leadId?: string }) {
  ...
  function moveSection(direction: -1 | 1) {
    ...
    const sections = [...specification.sections]; const index = sections.indexOf(selectedSection); const next = index + direction;
    if (index < 0 || next < 0 || next >= sections.length) return;
    [sections[index], sections[next]] = [sections[next], sections[index]];
    updateSpec(applyWebsiteSpecificationPatch(specification, { kind: 'section_order', sections }));
  }
