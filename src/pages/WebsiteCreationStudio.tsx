...
import { AlertCircle, ArrowRight, Check, Loader2, Sparkles, WandSparkles } from 'lucide-react';
...
export default function WebsiteCreationStudio({ creationProjectId, leadId }: { creationProjectId?: string; leadId?: string }) {
        ...
        if (resolvedLeadId && isAuthenticated) {
          ...
          if (!leadResult.error && leadResult.data) {
            ...
            if (!businessResult.error && businessResult.data) {
              setBusiness((current) => ({ ...current, businessName: businessResult.data.name || '', industry: businessResult.data.industry || '', email: businessResult.data.email || '', phone: businessResult.data.phone || '', specialRequirements: leadResult.data.requirements || '' }));
            }
          }
        }
      } catch (err) {
        if (mounted) setError(err instanceof Error ? err.message : 'Unable to load Template Studio.');
  ...
  function toggleSection(section: WebsiteSectionId) {
  ...
  async function ensureProject() {
    ...
    const result = await supabase.rpc('create_creation_project', {
      p_type: 'website',
      p_title: `${business.businessName || 'Website'} Preview`,
      p_client_id: clientId,
      p_connector_id: connectorId,
      p_lead_id: resolvedLeadId || null,
      p_business_id: null,
      p_project_id: null,
      p_business_info: business,
      p_requested_sections: requestedSections,
    });
    if (result.error) throw result.error;
  ...
  async function generate() {
    ...
    try {
      ...
      setProject((current) => current ? { ...current, selected_template_id: selectedTemplate.id, requested_sections: spec.sections, specification: spec, status: 'preview' } : current);
      setNotice('Preview regenerated and saved to the creation project.');
  ...
  return <div className="space-y-8">
    ...
    <div className="grid gap-8 xl:grid-cols-[420px_1fr]">
      <div className="space-y-6 rounded-3xl border border-white/10 bg-white/[.03] p-6">
        <div><h2 className="font-semibold text-white">Business information</h2><p className="mt-1 text-xs text-gray-500">This information becomes the structured source for the preview.</p></div>
