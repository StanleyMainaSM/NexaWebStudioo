...
export default function WebsiteCreationStudio({ creationProjectId, leadId }: { creationProjectId?: string; leadId?: string }) {
  ...
  return <div className="space-y-5">{/* Workspace UI remains unchanged below this point. */}
    ...
    <div className="grid min-h-[calc(100vh-12rem)] overflow-hidden rounded-3xl border border-white/10 bg-[#0b0d12] xl:grid-cols-[250px_minmax(0,1fr)_320px]">
      ...
      <main className="min-w-0 border-b border-white/10 bg-[#151820] xl:border-b-0 xl:border-r"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-[#0f1117] px-4 py-3"><div className="flex items-center gap-1 rounded-xl border border-white/10 p-1"><button type="button" onClick={() => setPreviewMode('desktop')} aria-label="Desktop preview" className={`rounded-lg p-2 ${previewMode === 'desktop' ? 'bg-white/10 text-white' : 'text-gray-500'}`}><Monitor className="h-4 w-4" /></button><button type="button" onClick={() => setPreviewMode('tablet')} aria-label="Tablet preview" className={`rounded-lg p-2 ${previewMode === 'tablet' ? 'bg-white/10 text-white' : 'text-gray-500'}`}><Tablet className="h-4 w-4" /></button><button type="button" onClick={() => setPreviewMode('mobile')} aria-label="Mobile preview" className={`rounded-lg p-2 ${previewMode === 'mobile' ? 'bg-white/10 text-white' : 'text-gray-500'}`}><Smartphone className="h-4 w-4" /></button></div><div className="flex items-center gap-2 text-xs text-gray-500"><Eye className="h-4 w-4" />Live preview</div></div><div className="h-full overflow-auto p-4 md:p-6"><div className={`mx-auto w-full transition-all ${previewWidth}`}>{specification ? <WebsitePreviewRenderer spec={specification} /> : <div className="grid min-h-[620px] place-items-center rounded-3xl border border-dashed border-white/10 bg-white/[.02] text-center"><div><Sparkles className="mx-auto h-8 w-8 text-accent-400" /><h2 className="mt-4 text-xl font-semibold text-white">Start your website</h2><p className="mt-2 max-w-sm text-sm text-gray-500">Select a DB-backed template and generate a structured preview.</p></div></div>}</div></div></main>
      <aside className="min-w-0 overflow-y-auto p-4 md:p-5"><div className="flex items-center gap-2"><Palette className="h-4 w-4 text-accent-400" /><h2 className="text-sm font-semibold text-white">Properties</h2></div>{!specification ? <div className="mt-6 space-y-5"><h3 className="text-sm font-semibold text-white">Website details</h3><Field label="Business name" value={business.businessName} onChange={(value) => editBusiness('businessName', value)} /><Field label="Description" value={business.businessDescription || ''} onChange={(value) => editBusiness('businessDescription', value)} multiline /><Field label="Email" value={business.email || ''} onChange={(value) => editBusiness('email', value)} /><Field label="Phone" value={business.phone || ''} onChange={(value) => editBusiness('phone', value)} /></div> : <div className="mt-5 space-y-5">{selectedSection === 'navbar' && <><h3 className="text-sm font-semibold text-white">Navigation</h3>{specification.navigation.map((item) => <div key={item.section} className="space-y-2 rounded-xl border border-white/10 bg-white/[.02] p-3"><Field label={sectionLabels[item.section]} value={item.label} onChange={(value) => updateSpec(updateWebsiteNavigationItem(specification, item.section, { label: value, section: item.section }))} /><label className="block text-xs text-gray-500">Destination<select value={item.section} onChange={(event) => updateSpec(updateWebsiteNavigationItem(specification, item.section, { label: item.label, section: event.target.value as WebsiteSectionId }))} className="mt-1 w-full rounded-lg border border-white/10 bg-ink-950 px-2 py-2 text-white">{navTargets.map((target) => <option key={target} value={target}>{`#${target}`}</option>)}</select></label></div>)}</>}
