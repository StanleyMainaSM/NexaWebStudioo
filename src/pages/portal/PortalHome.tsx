...
export default function PortalHome() {
  ...
  if (canAccessConnector) {
    workspaceCards.push({
      ...
      description:
        'Manage your leads, submit new leads, and track your connector activity.',
      icon: ConnectorIcon,
  ...
  if (canAccessOperator) {
    workspaceCards.push({
      ...
      description:
        'Access assigned projects, tasks, progress, documents, and communication.',
      icon: Briefcase,
  ...
  if (canAccessClient) {
    workspaceCards.push({
      ...
      title: 'Client Portal',
      description:
        'View your projects, invoices, documents, activity, and account information.',
      icon: UserCircle2,
      path: '/portal',
    });
  }
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-400">
          Your Avelixa account has access to multiple workspaces.
        ...
        {workspaceCards.map((workspace) => {
          ...
          return (
            <button
              ...
              className="group text-left rounded-2xl border border-ink-800/50 bg-white/[0.03] p-6 transition-all hover:border-accent-500/40 hover:bg-white/[0.05]"
            >
              <div className="flex items-start justify-between gap-5">
                ...
                <ArrowRight className="h-5 w-5 text-gray-600 transition-transform group-hover:translate-x-1 group-hover:text-accent-400" />
              </div>
              ...
              <div className="mt-6 text-sm font-medium text-accent-400">
                Enter workspace
              </div>
            </button>
          );
        ...
        <div className="rounded-2xl border border-ink-800/50 bg-white/[0.02] p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {roles.map((role) => (
              <span
                key={role}
                className="rounded-full border border-accent-500/20 bg-accent-500/10 px-3 py-1.5 text-xs font-medium uppercase tracking-wider text-accent-300"
              >
                {role}
              </span>
            ))}
