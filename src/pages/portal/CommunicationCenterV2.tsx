...
import { ArrowLeft, Ban, BellOff, Check, CheckCheck, Clipboard, Download, Loader2, MessageCirclePlus, MessageSquare, MoreVertical, Phone, Plus, Search, Send, Settings2, Trash2, UserPlus, UserRound, Video, Volume2, X } from 'lucide-react';
...
export default function CommunicationCenterV2() {
  ...
  const messageTicks = (m: Message) => {
    ...
    if (m.read_at) return <CheckCheck className="h-3.5 w-3.5 text-sky-400" title="Read" />;
    if (presence?.is_online) return <CheckCheck className="h-3.5 w-3.5 text-gray-400" title="Delivered — recipient online" />;
    return <Check className="h-3.5 w-3.5 text-gray-500" title="Sent — recipient offline" />;
  };
    ...
    <div className="flex h-[calc(100vh-7rem)] min-h-[620px]">
      <aside className={`${selectedId ? 'hidden md:flex' : 'flex'} w-full md:w-[360px] shrink-0 flex-col border-r border-white/10 bg-ink-950/80`}>
        <div className="border-b border-white/10 p-4">
          <div className="flex items-center justify-between"><div><div className="text-lg font-semibold text-white">Messages</div><div className="mt-1 text-xs text-gray-500">Your conversations</div></div><button type="button" onClick={() => setShowPeople(v => !v)} className="rounded-xl border border-white/10 bg-white/5 p-2 text-gray-300 hover:bg-white/10" title="New chat"><MessageCirclePlus className="h-5 w-5" /></button></div>
