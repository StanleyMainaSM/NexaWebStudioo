...
const STORAGE_KEY = 'avelixa_active_workspace';
...
function readStoredWorkspace(roles: string[]): WorkspaceKey | null {
  if (typeof window === 'undefined') return null;
  const value = window.sessionStorage.getItem(STORAGE_KEY) as WorkspaceKey | null;
...
function getCurrentWorkspace(pathname: string, roles: string[]): WorkspaceKey {
  ...
  const stored = readStoredWorkspace(roles);
  if (stored) return stored;

  if (pathname.startsWith('/portal/clients') || pathname.startsWith('/portal/reviews') || pathname.startsWith('/portal/portfolio') || pathname.startsWith('/portal/services') || pathname.startsWith('/portal/website-packages') || pathname.startsWith('/portal/maintenance') || pathname.startsWith('/portal/hosting') || pathname.startsWith('/portal/revenue')) {
...
export default function PortalLayout() {
  ...
  useEffect(() => {
    ...
    const explicitWorkspace =
      location.pathname.startsWith('/portal/owner') ? 'owner' :
      location.pathname.startsWith('/portal/admin') ? 'admin' :
      location.pathname.startsWith('/portal/connector') ? 'connector' :
      location.pathname.startsWith('/portal/operator') ? 'operator' :
      null;

    if (explicitWorkspace && normalizedRoles.includes(explicitWorkspace)) {
      window.sessionStorage.setItem(STORAGE_KEY, explicitWorkspace);
    }
  }, [location.pathname, normalizedRoles.join('|')]);

  const availableWorkspaces = (['client', 'connector', 'operator', 'admin', 'owner'] as WorkspaceKey[])
    .filter((key) => normalizedRoles.includes(key))
    .map((key) => workspaceDefinitions[key]);

  const switchWorkspace = (workspace: Workspace) => {
    if (!normalizedRoles.includes(workspace.key)) return;
    if (typeof window !== 'undefined') window.sessionStorage.setItem(STORAGE_KEY, workspace.key);
  ...
  const workspacePath = (path: string) => {
    if (currentWorkspaceKey === 'client') return path;
    return path === '/portal' ? currentWorkspace.path : path;
  };

  const dashboardPath = currentWorkspace.path;
  const navItems: NavItem[] = [
    { name: 'Dashboard', path: dashboardPath, icon: LayoutDashboard, roles: 'all' },
    { name: 'Projects', path: workspacePath('/portal/projects'), icon: FolderKanban, roles: 'all' },
    { name: 'Invoices', path: workspacePath('/portal/invoices'), icon: ReceiptText, roles: 'all' },
    { name: 'Documents', path: workspacePath('/portal/documents'), icon: FileText, roles: 'all' },
    { name: 'Messages', path: workspacePath('/portal/messages'), icon: MessageSquare, roles: 'all' },
    { name: 'Activity', path: workspacePath('/portal/activity'), icon: BellRing, roles: 'all' },
    ...
    { name: 'Settings', path: workspacePath('/portal/settings'), icon: Settings, roles: 'all' },
  ];
    ...
    return (
      <div className="rounded-2xl border border-accent-500/20 bg-accent-500/5 p-4">
        ...
        <div className="mt-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-accent-500/10 border border-accent-500/20 flex items-center justify-center shrink-0"><CurrentIcon className="w-5 h-5 text-accent-400" /></div>
          <div className="min-w-0"><div className="text-sm font-semibold text-white">{currentWorkspace.label} Portal</div><div className="text-xs text-gray-500 mt-0.5">{availableWorkspaces.length} workspaces available</div></div>
        </div>
        <div className="mt-4 space-y-2">
          {otherWorkspaces.map((workspace) => { const WorkspaceIcon = workspace.icon; return (
            <button key={workspace.key} type="button" onClick={() => switchWorkspace(workspace)} className="w-full inline-flex items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-xs font-semibold text-gray-300 transition-all hover:border-accent-500/30 hover:bg-accent-500/10 hover:text-accent-300">
              <span className="inline-flex items-center gap-2"><WorkspaceIcon className="w-4 h-4" />Switch to {workspace.label}</span><ArrowLeftRight className="w-4 h-4" />
            </button>
          ); })}
        </div>
      </div>
  ...
  const renderNav = () => (
    <nav className="space-y-1">
      {visibleNavItems.map((item) => {
        ...
        return <Link key={item.name} to={item.path} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${active ? 'text-accent-400 bg-accent-500/10 border border-accent-500/10' : 'text-gray-400 hover:text-accent-400 hover:bg-white/5 border border-transparent'}`}><item.icon className="w-5 h-5 shrink-0" />{item.name}</Link>;
      })}
    </nav>
  );
    ...
    <div className="min-h-screen bg-ink-950 flex overflow-hidden">
      <aside className="hidden md:flex w-64 flex-col border-r border-ink-800/50 bg-ink-950 z-20 shrink-0">
        <div className="h-20 flex items-center px-6 border-b border-ink-800/50 shrink-0">
          <Link to={currentWorkspace.path} className="flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center overflow-hidden shrink-0"><img src="/logo.avif" alt="Avelixa" className="w-full h-full object-contain" /></div>
            <div><div className="font-medium tracking-tight text-white text-lg">Avelixa</div><div className="text-[9px] uppercase tracking-[0.2em] text-accent-400 mt-0.5">{currentWorkspace.label} Portal</div></div>
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-ink-800/50">
          <div className="flex items-center gap-3 px-4 py-3"><div className="w-10 h-10 rounded-full bg-accent-500/10 border border-accent-500/20 flex items-center justify-center text-accent-400 font-semibold uppercase shrink-0">{userInitial}</div><div className="overflow-hidden min-w-0"><div className="text-sm font-medium text-white truncate">{user?.email || 'Avelixa User'}</div><div className="text-[10px] text-accent-500 uppercase tracking-widest truncate mt-0.5">{currentWorkspace.label} Portal</div></div></div>
          <button type="button" onClick={handleLogout} className="mt-2 w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-400 hover:bg-red-500/10 transition-colors"><LogOut className="w-5 h-5" />Sign out</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-20 flex items-center justify-between px-5 border-b border-ink-800/50 md:hidden bg-ink-950/90 backdrop-blur-md z-20 shrink-0">
          <Link to={currentWorkspace.path} className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center overflow-hidden shrink-0"><img src="/logo.avif" alt="Avelixa" className="w-full h-full object-contain" /></div><div><div className="font-medium tracking-tight text-white text-lg">Avelixa</div><div className="text-[10px] uppercase tracking-widest text-accent-400">{currentWorkspace.label} Portal</div></div></Link>
          <button type="button" onClick={() => setSidebarOpen(true)} className="text-white p-2 rounded-xl hover:bg-white/5 transition-colors" aria-label="Open portal menu"><Menu className="w-6 h-6" /></button>
        </header>
      ...
      {sidebarOpen && <div className="fixed inset-0 z-50 flex md:hidden">
        ...
        <div className="relative w-72 max-w-[85vw] bg-ink-900 h-full flex flex-col border-r border-ink-800/50 shadow-2xl">
          <div className="h-20 flex items-center justify-between px-5 border-b border-ink-800/50"><Link to={currentWorkspace.path} onClick={() => setSidebarOpen(false)} className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center overflow-hidden"><img src="/logo.avif" alt="Avelixa" className="w-full h-full object-contain" /></div><div><div className="font-medium tracking-tight text-white text-lg">Avelixa</div><div className="text-[9px] uppercase tracking-[0.2em] text-accent-400">{currentWorkspace.label} Portal</div></div></Link><button type="button" onClick={() => setSidebarOpen(false)} className="text-gray-400 hover:text-white p-2 rounded-xl hover:bg-white/5 transition-colors" aria-label="Close portal menu"><X className="w-6 h-6" /></button></div>
          <div className="px-4 pt-5">{renderWorkspaceSwitcher()}</div>
          <div className="flex-1 overflow-y-auto py-6 px-4">{renderNav()}</div>
