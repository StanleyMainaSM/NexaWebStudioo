...
function roleTone(role: string) {
  switch (role) {
    ...
    default:
      return 'border-ink-700 bg-white/5 text-gray-400';
  }
}
...
export default function OwnerUserManagement() {
  ...
  useEffect(() => {
    ...
    async function checkSession() {
      try {
        ...
        if (mounted) {
          setError(
            `Unable to verify your session: ${getErrorMessage(err)}`
          );
        }
      } finally {
        if (mounted) {
  ...
  const verifyOwnerPassword = async (
    ...
    try {
      ...
      /*
       * Re-authentication is intentionally performed against the
       * currently signed-in user's email. The password is never
       * stored by this page.
       */
      ...
      if (signInError) {
        throw new Error(
          'The password is incorrect. Please enter the password you used to sign in.'
        );
      }
      ...
      console.error(
        'Owner user-management re-authentication failed:',
        err
      );
  ...
  const loadUsers = async (showSpinner = true) => {
    ...
    try {
      ...
      if (!contentType.includes('application/json')) {
        ...
        throw new Error(
          `The server returned an unexpected response (${response.status}). ${text.slice(
            0,
            120
          )}`
        );
      ...
      console.error(
        'Owner users could not be loaded:',
        err
      );

      setError(
        `Users could not be loaded: ${getErrorMessage(err)}`
      );
    } finally {
  ...
  const counts = useMemo(() => {
    return {
      ...
      connectors: users.filter((user) =>
        user.roles.includes('connector')
      ).length,
  ...
  const handleAddUser = async (
    ...
    try {
      ...
      console.error(
        'Owner user creation failed:',
        err
      );

      setError(
        `User could not be created: ${getErrorMessage(err)}`
      );
    } finally {
      setSavingUser(false);
    }
  ...
  const handleAddRole = async (
    ...
    try {
      ...
      setError(
        `Role could not be added: ${getErrorMessage(err)}`
      );
    } finally {
  ...
  const handleRemoveRole = async (
    ...
    const confirmed = window.confirm(
      `Remove the ${roleLabel(selectedRole)} role from ${
    ...
    try {
      ...
      const response = await fetch(
        `/api/owner/users/${user.id}/roles/${selectedRole}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setError(
        `Role could not be removed: ${getErrorMessage(err)}`
      );
    } finally {
      setActionUserId(null);
    }
  ...
  const handleDeleteUser = async (
    ...
    const confirmed = window.confirm(
      `DELETE ${user.full_name || user.email}?\n\nThis permanently removes the user's Avelixa account. This action cannot be undone.`
    );
    ...
    try {
      ...
      setError(
        `User could not be deleted: ${getErrorMessage(err)}`
      );
    } finally {
      setActionUserId(null);
    }
        ...
        <div className="w-full rounded-3xl border border-purple-500/20 bg-ink-950/80 p-8 shadow-2xl">
          ...
          <div className="mt-6 text-center">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-gray-400">
              This area contains high-level account controls.
              For your security, please re-enter your Owner
              account password before continuing.
            </p>
          </div>

          {error && (
            <div className="mt-6 flex items-start gap-3 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm leading-6 text-red-300">
          ...
          >
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  autoFocus
                  className="w-full rounded-xl border border-ink-800/60 bg-white/5 px-4 py-3 pr-12 text-sm text-white outline-none placeholder:text-gray-700 focus:border-purple-500/40"
                />
            ...
            <div className="rounded-xl border border-purple-500/10 bg-purple-500/5 p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs leading-5 text-gray-400">
                  Your password is used only to verify your
                  current Supabase account. It is not stored
                  by Avelixa or sent to the Avelixa server.
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-purple-500 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {verifyingPassword ? (
                <>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-400">
                Manage Avelixa accounts, roles, and access.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={refreshing}
            className="inline-flex items-center gap-2 rounded-xl border border-ink-800/50 bg-white/5 px-4 py-2.5 text-sm font-medium text-gray-300 transition-colors hover:bg-white/10 hover:text-white disabled:opacity-50"
          >
            <RefreshCw
              className={`h-4 w-4 ${
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-5 py-4">
        ...
        <div>
          ...
          <div className="mt-1 text-xs text-gray-500">
            Your Owner password was verified for this session.
          </div>
        </div>
      </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-ink-800/50 bg-ink-950/70">
        <div className="flex flex-col gap-4 border-b border-ink-800/50 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-3">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Add users, assign roles, remove roles, or permanently delete accounts.
            </p>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-accent-500/20 bg-accent-500/10 px-4 py-2.5 text-sm font-semibold text-accent-300 transition-colors hover:bg-accent-500/20"
          >
                          ...
                          {user.roles.length === 0 ? (
                            ...
                            user.roles.map(
                              (userRole) => (
                                <span
                                  key={userRole}
                                  className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider ${roleTone(
                                    userRole
                                  )}`}
                                >
                                  {roleLabel(
                          ...
                          {!isOwner && (
                            <>
                              <select
                                ...
                                className="rounded-xl border border-ink-800/60 bg-white/5 px-3 py-2 text-xs text-gray-300 outline-none focus:border-accent-500/40"
                              >
                                <option
                                  value=""
                              ...
                              <button
                                ...
                                className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-red-500/20 bg-red-500/5 text-red-400 transition-colors hover:bg-red-500/10 disabled:opacity-50"
                              >
                                {isBusy ? (
                                  <Loader2 className="h-4 w-4 animate-spin" />
                                ) : (
      ...
      {showAddUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-ink-800/60 bg-ink-950 p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Create an account and assign its initial role.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="inline-flex h-9 w-9 items-center justify-center rounded-xl text-gray-500 hover:bg-white/5 hover:text-white"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setRole(
                      event.target.value as AllowedRole
                    )
                  }
                  disabled={savingUser}
              ...
              <div className="rounded-xl border border-accent-500/10 bg-accent-500/5 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-xs leading-5 text-gray-400">
                    ...
                    and will create their own password. Avelixa
                    will not store or send a plaintext password.
                  </p>
                </div>
              </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-gray-400">
              User management actions remain authorized on the
              Avelixa server using the authenticated Owner role.
              The browser never receives the Supabase service-role
              key.
            </p>
          </div>
        </div>
