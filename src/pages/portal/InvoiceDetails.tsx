...
import { validatePaymentAmount } from '../../lib/paymentAmountValidation';
...
export default function InvoiceDetails() {
  ...
  const paymentAmountValidation = useMemo(() => validatePaymentAmount(paymentAmount, paymentSummary.balance), [paymentAmount, paymentSummary.balance]);

  useEffect(() => {
    if (canSubmitPayment && !paymentAmount) {
      setPaymentAmount(String(paymentSummary.balance));
      setPaymentAmountTouched(false);
    }
  }, [canSubmitPayment, paymentAmount, paymentSummary.balance]);
  ...
  const submitPayment = async (event: FormEvent) => {
    ...
    if (!paymentAmountValidation.valid) { setPaymentAmountTouched(true); setPaymentError(paymentAmountValidation.message); return; }
    if (!referenceNumber.trim()) { setPaymentError('Enter the payment reference number before submitting.'); return; }
    setSubmitting(true);
    try {
      const { error: submitError } = await supabase.rpc('submit_invoice_payment', { p_invoice_id: invoice.id, p_amount: paymentAmountValidation.amount, p_payment_method: paymentMethod, p_reference_number: referenceNumber.trim() });
      if (submitError) throw submitError;
      setPaymentAmount(''); setPaymentAmountTouched(false); setReferenceNumber(''); setPaymentSuccess('Payment submitted successfully. It is now awaiting Owner/Admin verification.'); await loadInvoice();
  ...
  return <div className="space-y-6">
    ...
    {canSubmitPayment && <div className="glass rounded-2xl border border-accent-500/20 p-6 sm:p-8"><div className="flex items-center gap-3"><CreditCard className="w-5 h-5 text-accent-500" /><div><h3 className="text-lg font-medium text-white">Submit payment</h3><p className="mt-1 text-sm text-gray-400">Enter the amount you are paying and submit the payment reference for Owner/Admin verification.</p></div></div>{paymentError && <div className="mt-5 rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-300">{paymentError}</div>}{paymentSuccess && <div className="mt-5 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-emerald-300">{paymentSuccess}</div>}<form onSubmit={submitPayment} className="mt-6 grid gap-4 md:grid-cols-2"><div><label htmlFor="payment-amount" className="block text-sm text-gray-400 mb-2">Amount</label><input id="payment-amount" type="number" min="0.01" max={paymentSummary.balance} step="0.01" value={paymentAmount} onChange={event => { setPaymentAmount(event.target.value); setPaymentAmountTouched(true); setPaymentError(''); }} className="w-full rounded-xl bg-ink-950 border border-ink-800 px-4 py-3 text-white outline-none focus:border-accent-500" aria-invalid={paymentAmountTouched && !paymentAmountValidation.valid} aria-describedby="payment-amount-help" />{paymentAmountTouched && !paymentAmountValidation.valid && <p className="mt-2 text-xs text-red-300">{paymentAmountValidation.message}</p>}<p id="payment-amount-help" className="mt-2 text-xs text-gray-500">Remaining balance: {formatCurrency(paymentSummary.balance)}. You may pay the full balance or a smaller amount.</p></div><div><label className="block text-sm text-gray-400 mb-2">Payment method</label><select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value)} className="w-full rounded-xl bg-ink-950 border border-ink-800 px-4 py-3 text-white outline-none focus:border-accent-500"><option value="mpesa">M-Pesa</option><option value="bank_transfer">Bank transfer</option><option value="card">Card</option><option value="cash">Cash</option><option value="other">Other</option></select></div><div className="md:col-span-2"><label className="block text-sm text-gray-400 mb-2">Payment reference</label><input value={referenceNumber} onChange={event => setReferenceNumber(event.target.value)} placeholder="e.g. M-Pesa transaction code" className="w-full rounded-xl bg-ink-950 border border-ink-800 px-4 py-3 text-white outline-none focus:border-accent-500" /></div><div className="md:col-span-2 flex justify-end"><button type="submit" disabled={submitting || !referenceNumber.trim() || !paymentAmountValidation.valid} className="inline-flex items-center gap-2 rounded-xl bg-accent-600 px-5 py-3 text-sm font-medium text-white hover:bg-accent-500 disabled:opacity-50">{submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <CreditCard className="w-4 h-4" />}{submitting ? 'Submitting...' : 'Submit payment'}</button></div></form></div>}
