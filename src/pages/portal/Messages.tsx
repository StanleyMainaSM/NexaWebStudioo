...
import {
  ArrowLeft, Ban, BellOff, Check, CheckCheck, ChevronDown, Clipboard, Download,
  FileText, Loader2, MessageCirclePlus, MessageSquare, MoreVertical, Phone,
  PhoneOff, Plus, Search, Send, ShieldCheck, Trash2, UserPlus, UserRound, Video,
  X, Volume2
} from 'lucide-react';

type Person = {
  user_id: string; full_name: string | null; email: string | null; role_context: string | null;
  connector_id: string | null; is_online: boolean; last_seen_at: string | null; avatar_url?: string | null;
};
...
function Avatar({ person, size = 'h-11 w-11' }: { person?: Person | null; size?: string }) {
  ...
  return <div className={`${size} relative shrink-0 overflow-hidden rounded-full border border-white/10 bg-accent-500/10 flex items-center justify-center text-accent-300 font-semibold`}>
    {person?.avatar_url ? <img src={person.avatar_url} alt="" className="h-full w-full object-cover" /> : initial}
    {person?.is_online && <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-ink-950 bg-emerald-400" />}
  </div>;
}
...
export default function Messages() {
  ...
  const refreshDirectory = async () => {
    if (!user) return;
    const [{ data: convoRows }, { data: contactRows }] = await Promise.all([
      supabase.rpc('list_direct_conversations'),
      supabase.rpc('list_communication_contacts')
    ]);
    ...
    if (ids.length) {
      ...
      const merged = ids.map(id => { const p = pm.get(id); const u = um.get(id); return { user_id: id, full_name: p?.full_name || null, email: p?.email || null, avatar_url: p?.avatar_url || null, role_context: null, connector_id: null, is_online: !!u?.is_online, last_seen_at: u?.last_seen_at || null }; });
      setPeople(merged);
    } else setPeople([]);
  };
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        await supabase.rpc('communication_set_presence', { p_online: true });
        await refreshDirectory();
        const { data: prefs } = await supabase.from('user_notification_preferences').select('message_notifications').eq('user_id', user.id).maybeSingle();
        if (prefs?.message_notifications === false) { /* UI still shows unread state; browser sound is suppressed elsewhere. */ }
  ...
  const loadChat = async (conversationId: string) => {
    ...
    if (msgError) setError(errText(msgError));
    else setMessages((msgRows || []) as Message[]);
    if (callError) setError(errText(callError)); else setCalls((callRows || []) as CallSession[]);
    setMuted(!!pref?.muted);
    const person = people.find(p => p.user_id === selectedConversation?.other_user_id);
    if (person) setSelectedPerson(person);
    setLoadingChat(false);
  };

  useEffect(() => { if (selectedId) void loadChat(selectedId); else { setMessages([]); setCalls([]); setSelectedPerson(null); } }, [selectedId]);
    ...
    const channel = supabase.channel(`avelixa-messages-${user.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'direct_messages' }, payload => {
        ...
        if (m.sender_id !== user.id) {
          ...
          if (m.conversation_id === selectedId) void markRead(m.conversation_id);
          if (lastMessageId.current !== m.id && !muted) void playMessageSound();
          lastMessageId.current = m.id;
        }
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'direct_messages' }, payload => {
        const m = payload.new as Message; setMessages(cur => cur.map(x => x.id === m.id ? m : x));
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'call_sessions' }, payload => {
        const c = payload.new as CallSession;
        if (c.callee_id === user.id && c.status === 'ringing') setIncomingCall(c);
        if (c.direct_conversation_id === selectedId) setCalls(cur => [...cur.filter(x => x.id !== c.id), c]);
      })
  ...
  const markRead = async (conversationId: string) => {
    ...
    const { error: e } = await supabase.from('direct_messages').update({ read_at: now }).in('id', unread);
    if (!e) setMessages(cur => cur.map(m => unread.includes(m.id) ? { ...m, read_at: now } : m));
  };
  ...
  const startConversation = async (person: Person) => {
    ...
    try {
      const { data, error: e } = await supabase.rpc('get_or_create_direct_conversation', { p_recipient_id: person.user_id });
      if (e) throw e;
      if (!data) throw new Error('Avelixa could not create the conversation.');
      await supabase.rpc('communication_add_contact', { p_contact_user_id: person.user_id });
      await refreshDirectory(); setSelectedPerson(person); setSelectedId(data as string); setNewChatOpen(false); setNewChatQuery('');
  ...
  const sendMessage = async (e: FormEvent) => {
    ...
    setBusy(true); setError(null);
    try {
      const content = replyTo ? `↩ ${nameOf(selectedPersonFromDirectory)}: ${replyTo.content}\n\n${messageText.trim()}` : messageText.trim();
      const { data, error: insertError } = await supabase.from('direct_messages').insert({ conversation_id: selectedId, sender_id: user.id, content }).select('id,conversation_id,sender_id,content,read_at,created_at').single();
      if (insertError) throw insertError;
      setMessages(cur => cur.some(m => m.id === data.id) ? cur : [...cur, data as Message]); setMessageText(''); setReplyTo(null); void refreshDirectory();
    } catch (e) { setError(errText(e)); } finally { setBusy(false); }
  };

  const sendTyping = (value: boolean) => {
    if (!selectedId || !user) return;
    const channel = supabase.channel(`avelixa-typing-${selectedId}`); void channel.send({ type: 'broadcast', event: 'typing', payload: { userId: user.id, conversationId: selectedId, typing: value } });
    if (typingTimer.current) window.clearTimeout(typingTimer.current);
    if (value) typingTimer.current = window.setTimeout(() => sendTyping(false), 1600);
  };
  ...
  const startCall = async (type: 'voice' | 'video') => {
    ...
    setBusy(true); setError(null);
    try {
      const { data: allowed, error: allowError } = await supabase.rpc('communication_can_contact', { p_other_user_id: selectedConversation.other_user_id });
      if (allowError) throw allowError; if (!allowed) throw new Error('This user is unavailable for communication.');
      const { data, error: e } = await supabase.from('call_sessions').insert({ direct_conversation_id: selectedId, caller_id: user.id, callee_id: selectedConversation.other_user_id, call_type: type, status: 'ringing' }).select('id,direct_conversation_id,caller_id,callee_id,call_type,status,duration_seconds,created_at,started_at,ended_at').single();
      if (e) throw e;
      const c = data as CallSession; setCalls(cur => [...cur, c]); setActiveCall({ id: c.id, callType: type, callerId: c.caller_id, calleeId: c.callee_id, remoteName: nameOf(selectedPersonFromDirectory), isIncoming: false, directConversationId: selectedId });
    } catch (e) { setError(`Could not start call: ${errText(e)}`); } finally { setBusy(false); }
  };

  const acceptIncoming = (c: CallSession) => {
    const p = people.find(x => x.user_id === c.caller_id); setSelectedPerson(p || null); setSelectedId(c.direct_conversation_id); setIncomingCall(null);
    setActiveCall({ id: c.id, callType: c.call_type, callerId: c.caller_id, calleeId: c.callee_id, remoteName: nameOf(p), isIncoming: true, directConversationId: c.direct_conversation_id });
  };
  ...
  const blockToggle = async () => {
    ...
    setBusy(true); setError(null);
    try {
      const { data: rows } = await supabase.from('user_blocks').select('id').eq('blocker_id', user?.id || '').eq('blocked_id', selectedConversation.other_user_id).limit(1);
      if (rows?.length) await supabase.rpc('communication_unblock_user', { p_blocked_id: selectedConversation.other_user_id }); else await supabase.rpc('communication_block_user', { p_blocked_id: selectedConversation.other_user_id });
      setBlocked(!rows?.length); setNotice(rows?.length ? 'User unblocked.' : 'User blocked.');
    } catch (e) { setError(errText(e)); } finally { setBusy(false); }
  };
  ...
  const clearChat = async () => {
    ...
    setBusy(true);
    try { const { error: e } = await supabase.from('conversation_preferences').upsert({ user_id: user.id, conversation_id: selectedId, cleared_at: new Date().toISOString() }, { onConflict: 'user_id,conversation_id' }); if (e) throw e; setMessages([]); setCalls([]); setNotice('Chat cleared for you.'); } catch (e) { setError(errText(e)); } finally { setBusy(false); }
  };
  ...
  const exportChat = () => {
    if (!selectedConversation) return;
    const person = nameOf(selectedPersonFromDirectory); const lines = [`Avelixa chat with ${person}`, `Exported ${new Date().toLocaleString()}`, ''];
    messages.forEach(m => lines.push(`[${dateOf(m.created_at)} ${timeOf(m.created_at)}] ${m.sender_id === user?.id ? 'You' : person}: ${m.content}`));
    calls.forEach(c => lines.push(`[${dateOf(c.created_at)} ${timeOf(c.created_at)}] ${c.call_type === 'video' ? 'Video' : 'Voice'} call — ${c.status}${c.duration_seconds ? ` (${c.duration_seconds}s)` : ''}`));
  ...
  const conversationItems = useMemo(() => {
    ...
    return conversations.filter(c => !q || nameOf({ full_name: c.other_full_name, email: c.other_email }).toLowerCase().includes(q) || c.other_email?.toLowerCase().includes(q));
  }, [conversations, search]);
  const chatMessages = useMemo(() => { const q = conversationSearch.trim().toLowerCase(); return q ? messages.filter(m => m.content.toLowerCase().includes(q)) : messages; }, [messages, conversationSearch]);
  const unreadTotal = useMemo(() => messages.filter(m => m.sender_id !== user?.id && !m.read_at).length, [messages, user?.id]);
  const newPeople = useMemo(() => { const q = newChatQuery.trim().toLowerCase(); const source = contacts.length ? contacts : people; return source.filter(p => p.user_id !== user?.id && (!q || nameOf(p).toLowerCase().includes(q) || p.email?.toLowerCase().includes(q) || p.connector_id?.toLowerCase().includes(q))).slice(0, 30); }, [contacts, people, newChatQuery, user?.id]);
