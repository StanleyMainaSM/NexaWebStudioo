import type { BusinessInformation, WebsiteSectionId, WebsiteSpecification, WebsiteTemplate, WebsiteTheme } from './types.ts';
...
export function generateWebsiteSpecification(business: BusinessInformation, template: WebsiteTemplate, requestedSections: WebsiteSectionId[] = [], attributionEnabled = true): WebsiteSpecification {
  ...
  return { version: 1, template: { id: template.id, slug: template.slug, name: template.name, visual_style: template.visual_style }, business: { ...business, businessName: name, industry: clean(business.industry), businessDescription: description, services, products, targetAudience: clean(business.targetAudience), location: clean(business.location), phone: clean(business.phone), email: clean(business.email), whatsapp: clean(business.whatsapp), specialRequirements: clean(business.specialRequirements) }, sections, theme: themeFor(template, business), navigation: sections.filter((section) => !['navbar', 'footer', 'hero'].includes(section)).map((section) => ({ label: navigationLabel[section], section })), content: { hero: { eyebrow: clean(business.industry), title: name, subtitle: description, cta: 'Get in touch' }, about: { title: `About ${name}`, body: description }, services: { items: services.length ? services : ['Professional service', 'Customer support', 'Tailored solutions'] }, products: { items: products.length ? products : ['Featured product'] }, gallery: { images: business.imagery || [] }, testimonials: { items: [{ quote: 'A trusted local business experience.', author: 'Customer' }] }, pricing: { items: [] }, faq: { items: [{ question: 'How can I get started?', answer: 'Contact the business using the details below.' }] }, contact: { phone: business.phone || '', email: business.email || '', whatsapp: business.whatsapp || '' }, location: { address: business.location || '' }, footer: { businessName: name } }, attribution: { enabled: attributionEnabled, label: 'Made with Avelixa' } };
}
