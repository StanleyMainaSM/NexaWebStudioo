...
function isWebsiteSectionId(value: unknown): value is WebsiteSectionId {
  return typeof value === 'string' && (WEBSITE_SECTIONS as readonly string[]).includes(value);
}

function validThemeValue(value: unknown, field: string, errors: string[]) {
...
export function validateWebsiteSpecification(specification: WebsiteSpecification, template?: WebsiteTemplate | null): string[] {
  ...
  if (!Array.isArray(specification.sections) || specification.sections.length === 0) {
    ...
    for (const section of specification.sections) {
      if (!isWebsiteSectionId(section)) errors.push(`Unsupported website section: ${String(section)}.`);
  ...
  if (!Array.isArray(specification.navigation)) {
    errors.push('Website navigation is invalid.');
  } else {
    const seen = new Set<string>();
    for (const item of specification.navigation) {
      if (!isRecord(item) || !clean(String(item.label || '')) || !isWebsiteSectionId(item.section)) {
        errors.push('Website navigation contains an invalid item.');
        continue;
      }
      if (!specification.sections.includes(item.section)) errors.push(`Navigation target is not enabled: ${item.section}.`);
      if (seen.has(item.section)) errors.push(`Duplicate navigation target: ${item.section}.`);
