...
async function getVapidPublicKey(): Promise<string> {
  ...
  try {
    data =
      JSON.parse(rawText) as PushSubscriptionResponse;
  } catch {
    throw new Error(
      `Avelixa push worker returned an invalid response (${response.status}).`
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      data.error ||
        `Avelixa push worker returned HTTP ${response.status}.`
    );
  }

  if (!data.publicKey) {
    throw new Error(
      'Avelixa push worker did not return a VAPID public key.'
    );
  }
...
async function getCurrentUser() {
  ...
  if (error) {
    throw new Error(
      `Unable to read the current Avelixa account: ${error.message}`
    );
  }
...
async function getServiceWorkerRegistration(): Promise<ServiceWorkerRegistration> {
  ...
  try {
    ...
    throw new Error(
      `Avelixa service worker registration failed: ${message}`
    );
  }
}
...
export async function subscribeToAvelixaPushNotifications(): Promise<PushSubscription | null> {
  ...
  if (!registration) {
    throw new Error(
      'Avelixa service worker registration is unavailable.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'The browser created an incomplete Avelixa push subscription.'
    );
  }
  ...
  if (subscriptionError) {
    throw new Error(
      `Avelixa could not save this device subscription: ${subscriptionError.message}`
    );
  }
...
export async function unsubscribeFromAvelixaPushNotifications(): Promise<void> {
  ...
  if (subscriptionError) {
    console.error(
      'Unable to remove Avelixa push subscription:',
      subscriptionError
    );
  }
  ...
  if (user) {
    ...
    if (preferenceError) {
      console.error(
        'Unable to update Avelixa notification preferences:',
        preferenceError
      );
    }
  }
...
export async function initializeAvelixaPushNotifications(): Promise<{
  ...
  if (!isPushSupported()) {
    throw new Error(
      'This browser does not support Avelixa push notifications.'
    );
  }
  ...
  if (!subscription) {
    throw new Error(
      'Avelixa could not create a push subscription for this device.'
    );
  }
