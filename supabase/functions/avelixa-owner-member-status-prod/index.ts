...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!ownerRole) {
      return json({ error: "Owner permission is required to manage members." }, 403);
    }
    ...
    if (!userId || typeof active !== "boolean") {
      return json({ error: "A target user and activation state are required." }, 400);
    }

    if (userId === actorId) {
      return json({ error: "The current Owner account cannot be deactivated from this interface." }, 400);
    }

    const { data: targetUser, error: targetUserError } = await admin.auth.admin.getUserById(userId);
    if (targetUserError || !targetUser.user) {
      return json({ error: "Target user account was not found." }, 404);
    }
    ...
    if (targetOwnerRole) {
      return json({ error: "Another Owner account cannot be deactivated from this interface." }, 400);
    }
    ...
    if (authStateError) {
      ...
      // Roll back the database state if Auth state could not be changed.
    ...
    if (!active) {
      await admin.from("connector_profiles").update({ is_active: false }).eq("user_id", userId);
    }
    ...
    if (auditError) {
      console.error("Owner member status audit error:", auditError);
    }
