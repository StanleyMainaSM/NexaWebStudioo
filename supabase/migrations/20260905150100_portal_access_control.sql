...
begin
  ...
  if not exists (select 1 from public.user_roles ur where ur.user_id = v_user_id and ur.role in ('owner', 'admin')) then return false; end if;
  if v_portal not in ('client','operator','connector','admin','owner') then return false; end if;
...
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$
declare v_portal text := lower(trim(p_portal)); v_password text := coalesce(p_password, ''); v_user_id uuid := auth.uid(); v_session_id uuid; v_password_hash text;
begin
...
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$
declare v_portal text := lower(trim(p_portal)); v_user_id uuid := auth.uid(); v_session_id uuid;
begin
  if v_user_id is null or v_portal not in ('client','operator','connector','admin','owner') then return false; end if;
  if not private.portal_access_role_allowed(v_user_id, v_portal) then return false; end if;
  begin v_session_id := (auth.jwt() ->> 'session_id')::uuid; exception when others then return false; end;
...
returns boolean language sql security definer
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$ select private.clear_portal_access(p_portal); $$;

create or replace function public.set_portal_access_password(p_portal text, p_password text)
returns boolean language sql security definer
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$ select private.set_portal_access_password(p_portal, p_password); $$;
