...
BEGIN
  ...
  SELECT
    cp.user_id,
    COALESCE(p.full_name, p.email, 'Connector')::text,
    p.email::text,
    cp.commission_rate,
    cp.is_active
  ...
  ORDER BY COALESCE(p.full_name, p.email, 'Connector');
END;
$$;
...
BEGIN
  ...
  IF p_connector_id IS NOT NULL AND p_commission_rate IS NOT NULL THEN
    SELECT commission_rate INTO v_profile_old_rate
...
CREATE OR REPLACE FUNCTION public.owner_update_connector_payout_reference(
  p_payout_id uuid,
  p_reference text,
  p_reason text DEFAULT NULL
)
...
BEGIN
  ...
  FROM public.payouts
  WHERE id = p_payout_id AND payout_type = 'connector_commission'
  FOR UPDATE;
  IF v_payout.id IS NULL THEN
    RAISE EXCEPTION 'Connector commission payout not found';
  END IF;
  IF v_payout.confirmation_status = 'confirmed' OR v_payout.status IN ('paid','completed') THEN
    RAISE EXCEPTION 'Confirmed connector payouts are locked';
  END IF;
  IF v_payout.confirmation_status <> 'sent' THEN
    RAISE EXCEPTION 'Payment reference can only be corrected after payment is sent and before connector confirmation';
  END IF;

  v_old_reference := v_payout.reference_number;
  UPDATE public.payouts
  SET reference_number = NULLIF(trim(p_reference), ''),
      updated_at = now()
  WHERE id = p_payout_id
  RETURNING * INTO v_payout;

  UPDATE public.commissions
  SET payment_reference = v_payout.reference_number,
      updated_at = now()
  ...
  VALUES (
    v_actor,
    'connector_payout_reference_changed',
    'payout',
    p_payout_id,
    jsonb_build_object('previous_reference', v_old_reference, 'new_reference', v_payout.reference_number, 'reason', NULLIF(trim(COALESCE(p_reason,'')),''))
  );
