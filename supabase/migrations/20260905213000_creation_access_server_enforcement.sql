...
BEGIN
  ...
  SELECT pap.password_hash INTO v_password_hash FROM private.portal_access_passwords pap WHERE pap.portal = v_portal;
  IF v_password_hash IS NULL OR crypt(v_password, v_password_hash) <> v_password_hash THEN RETURN false; END IF;
  ...
  ON CONFLICT (user_id, portal, session_id) DO UPDATE SET unlocked_at = now(), expires_at = now() + interval '8 hours';
  RETURN true;
END;
$$;
...
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, public, private, extensions, pg_temp
...
CREATE POLICY "Authorized users read creation projects" ON public.creation_projects FOR SELECT USING (
  public.has_portal_access('creation')
  AND (client_id = auth.uid() OR connector_id = auth.uid() OR operator_id = auth.uid() OR private.user_has_any_role(auth.uid(), ARRAY['owner','admin']))
);

DROP POLICY IF EXISTS "Authorized users update creation projects" ON public.creation_projects;
CREATE POLICY "Authorized users update creation projects" ON public.creation_projects FOR UPDATE
USING (
  public.has_portal_access('creation')
  AND (client_id = auth.uid() OR connector_id = auth.uid() OR operator_id = auth.uid() OR private.user_has_any_role(auth.uid(), ARRAY['owner','admin']))
)
...
CREATE POLICY "Users read own creation entitlement" ON public.creation_generation_entitlements FOR SELECT USING (
  public.has_portal_access('creation') AND (user_id = auth.uid() OR private.user_has_any_role(auth.uid(), ARRAY['owner','admin']))
);

DROP POLICY IF EXISTS "Users read own creation usage" ON public.creation_generation_usage;
CREATE POLICY "Users read own creation usage" ON public.creation_generation_usage FOR SELECT USING (
  public.has_portal_access('creation') AND (user_id = auth.uid() OR private.user_has_any_role(auth.uid(), ARRAY['owner','admin']))
);

CREATE OR REPLACE FUNCTION private.enforce_creation_access_on_project_mutation()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, private, extensions, pg_temp
AS $$
BEGIN
  IF auth.uid() IS NULL THEN RETURN NEW; END IF;
  IF NOT private.has_portal_access('creation') THEN RAISE EXCEPTION 'Website and template creation access required'; END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS enforce_creation_access_on_project_mutation ON public.creation_projects;
CREATE TRIGGER enforce_creation_access_on_project_mutation BEFORE INSERT OR UPDATE ON public.creation_projects FOR EACH ROW EXECUTE FUNCTION private.enforce_creation_access_on_project_mutation();
...
CREATE TRIGGER enforce_creation_access_on_output_mutation BEFORE INSERT OR UPDATE ON public.creation_generated_website_outputs FOR EACH ROW EXECUTE FUNCTION private.enforce_creation_access_on_output_mutation();
