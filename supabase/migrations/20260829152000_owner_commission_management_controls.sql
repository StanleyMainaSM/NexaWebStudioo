-- Protect the generic commission workflow from duplicate commission rows for the same
-- project/payment pair. Existing data was checked before adding this constraint.
CREATE UNIQUE INDEX IF NOT EXISTS commissions_project_payment_unique_idx
ON public.commissions(project_id, payment_id)
WHERE project_id IS NOT NULL AND payment_id IS NOT NULL;

-- Keep the Owner project finance read authoritative to the commission's stored
-- rate snapshot. The connector profile rate is only the fallback for projects
-- that do not yet have a commission.
