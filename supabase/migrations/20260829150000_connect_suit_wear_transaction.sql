...
BEGIN
  ...
  -- local/CI database has no production Auth user/profile, so the fixture
  -- portion must be a no-op there. The production role validation below is
  -- intentionally retained once that profile exists.
  IF NOT EXISTS (
    SELECT 1
