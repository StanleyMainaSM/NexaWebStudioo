...
-- Reuses the existing server-only portal password infrastructure.

DO $$
declare v_constraint text;
begin
  select conname into v_constraint from pg_constraint where conrelid = 'private.portal_access_passwords'::regclass and contype = 'c' and pg_get_constraintdef(oid) ilike '%portal%';
  if v_constraint is not null then execute format('alter table private.portal_access_passwords drop constraint %I', v_constraint); end if;
  select conname into v_constraint from pg_constraint where conrelid = 'private.portal_access_unlocks'::regclass and contype = 'c' and pg_get_constraintdef(oid) ilike '%portal%';
  if v_constraint is not null then execute format('alter table private.portal_access_unlocks drop constraint %I', v_constraint); end if;
end $$;

alter table private.portal_access_passwords add constraint portal_access_passwords_portal_check check (portal in ('client','operator','connector','admin','owner','creation'));
alter table private.portal_access_unlocks add constraint portal_access_unlocks_portal_check check (portal in ('client','operator','connector','admin','owner','creation'));

create or replace function private.portal_access_role_allowed(p_user_id uuid, p_portal text)
returns boolean language sql stable security definer
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$
  select exists (select 1 from public.user_roles ur where ur.user_id = p_user_id and lower(ur.role::text) = lower(p_portal))
  or (lower(p_portal) = 'creation' and exists (select 1 from public.user_roles ur where ur.user_id = p_user_id and lower(ur.role::text) in ('client','operator','connector','admin','owner')));
$$;
...
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$
declare v_portal text := lower(trim(p_portal)); v_user_id uuid := auth.uid();
begin
...
begin
  ...
  insert into private.portal_access_passwords(portal, password_hash, configured_at, updated_at, updated_by) values (v_portal, crypt(v_password, gen_salt('bf')), now(), now(), v_user_id)
...
begin
  ...
  if v_portal not in ('client','operator','connector','admin','owner','creation') then return false; end if;
  if length(v_new) < 12 then raise exception 'Portal access passwords must contain at least 12 characters.' using errcode = '22023'; end if;
  insert into private.portal_access_passwords(portal, password_hash, configured_at, updated_at, updated_by) values (v_portal, crypt(v_new, gen_salt('bf')), now(), now(), v_user_id)
  on conflict (portal) do update set password_hash = excluded.password_hash, updated_at = now(), updated_by = v_user_id;
  delete from private.portal_access_unlocks where portal = v_portal;
  return true;
end;
...
set search_path = pg_catalog, public, private, extensions, pg_temp
as $$
declare v_user_id uuid := auth.uid(); v_session_id uuid; v_portal text := nullif(lower(trim(p_portal)), '');
begin
  if v_user_id is null then return false; end if;
  ...
  if v_portal is not null and v_portal not in ('client','operator','connector','admin','owner','creation') then return false; end if;
  delete from private.portal_access_unlocks where user_id = v_user_id and session_id = v_session_id and (v_portal is null or portal = v_portal);
  return true;
end;
$$;
...
begin
  ...
  return query select p.portal, (pap.password_hash is not null), pap.configured_at, pap.updated_at from unnest(array['client','operator','connector','admin','owner','creation']::text[]) as p(portal) left join private.portal_access_passwords pap on pap.portal = p.portal order by case p.portal when 'client' then 1 when 'operator' then 2 when 'connector' then 3 when 'admin' then 4 when 'owner' then 5 when 'creation' then 6 end;
end;
$$;
