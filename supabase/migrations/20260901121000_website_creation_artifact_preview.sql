-- Make generated artifacts the addressable preview source while preserving the
-- existing public preview token and Creation Project authorization boundary.
...
DROP FUNCTION IF EXISTS public.get_public_creation_preview(UUID,TEXT);
...
BEGIN
  IF p_output_identity IS NULL THEN
    ...
    WHERE cp.public_preview_token = p_preview_token
      ...
      AND o.id = cp.latest_generated_output_identity
...
REVOKE ALL ON FUNCTION public.get_public_creation_preview(UUID,TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_creation_preview(UUID,TEXT) TO anon, authenticated;
