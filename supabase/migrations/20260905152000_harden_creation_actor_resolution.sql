-- Harden Creation Project authorization by resolving the authenticated actor
-- explicitly from the request JWT claims inside the SECURITY DEFINER RPC.
-- This preserves the existing authorization contract while avoiding reliance on
-- role-context behavior that can differ inside SECURITY DEFINER execution.
