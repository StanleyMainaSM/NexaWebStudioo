...
select t.id,v.avl_id,v.rate,v.active from t_ids t join (values
  ('connector_a','AVL-TEST-A',20.00,true),('connector_b','AVL-TEST-B',20.00,true),('inactive_connector','AVL-TEST-I',20.00,false)
...
select p.id,(select id from t_ids where name='client_a'),100000,'unpaid',current_date+30 from public.projects p where p.title='Release Test Project' returning id into t_invoice;
