...
-- The user-created trigger gives new users a client role. Replace roles where needed.
...
select results_eq(
  ...
  $$values(1::bigint)$$,
  'Invoice notification creates exactly one email-queue entry'
);

-- Client project-file protection.
select set_config('request.jwt.claims',jsonb_build_object('sub',(select id from t_ids where name='client_a')::text,'role','authenticated')::text,true);
set local role authenticated;
select lives_ok(
  $$insert into public.project_files(project_id,uploaded_by,file_name,storage_path,is_internal) select p.id,auth.uid(),'visible.txt','test/visible.txt',false from public.projects p where p.title='Release Test Project'$$,
  'Client can insert a non-internal project file'
);
select throws_ok(
  $$insert into public.project_files(project_id,uploaded_by,file_name,storage_path,is_internal) select p.id,auth.uid(),'internal.txt','test/internal.txt',true from public.projects p where p.title='Release Test Project'$$,
  '42501','Client cannot create an internal project file'
);

-- -----------------------------------------------------------------------------
-- Partial payment, authorization, cumulative settlement and commission
-- -----------------------------------------------------------------------------
select lives_ok(
  $$select public.submit_invoice_payment((select id from t_invoice),30000,'mpesa','TEST-30000')$$,
  'Client can submit a legitimate partial payment'
);
select results_eq(
  $$select count(*)::bigint from public.payments where invoice_id=(select id from t_invoice) and amount=30000 and status='pending'$$,
  $$values(1::bigint)$$,
  'Partial payment is stored as pending'
);
select throws_ok(
  $$select public.submit_invoice_payment((select id from t_invoice),0,'mpesa','TEST-ZERO')$$,
  NULL,'Zero payment is rejected'
);
select throws_ok(
  $$select public.submit_invoice_payment((select id from t_invoice),70001,'mpesa','TEST-OVER')$$,
  NULL,'Payment above remaining balance is rejected'
);
select throws_ok(
  $$select public.verify_invoice_payment((select id from public.payments where invoice_id=(select id from t_invoice) order by created_at desc limit 1),'completed','Client attempt')$$,
  NULL,'Client cannot verify a payment'
);
select throws_ok(
  $$insert into public.payments(invoice_id,amount,status) values((select id from t_invoice),1,'completed')$$,
  '42501','Client cannot directly manufacture a completed payment'
);

reset role;
select set_config('request.jwt.claims',jsonb_build_object('sub',(select id from t_ids where name='owner')::text,'role','authenticated')::text,true);
set local role authenticated;
select lives_ok(
  $$select public.verify_invoice_payment((select id from public.payments where invoice_id=(select id from t_invoice) and amount=30000 limit 1),'completed','Verified')$$,
  'Owner can verify a partial payment'
);
select results_eq(
  $$select status from public.invoices where id=(select id from t_invoice)$$,
  $$values('unpaid'::text)$$,
  'Invoice remains unpaid after a partial verified payment'
);
select results_eq(
  $$select count(*)::bigint from public.commissions where payment_id=(select id from public.payments where invoice_id=(select id from t_invoice) and amount=30000 limit 1)$$,
  $$values(1::bigint)$$,
  'Verified payment creates exactly one commission'
);
select results_eq(
  $$select eligible_amount,amount from public.commissions where payment_id=(select id from public.payments where invoice_id=(select id from t_invoice) and amount=30000 limit 1)$$,
  $$values(30000::numeric,6000::numeric)$$,
  'Commission follows the verified payment amount and 20 percent rate'
);
select lives_ok(
  $$update public.payments set status='completed' where id=(select id from public.payments where invoice_id=(select id from t_invoice) and amount=30000 limit 1)$$,
  'Reprocessing a completed payment remains safe'
);
select results_eq(
  $$select count(*)::bigint from public.commissions where payment_id=(select id from public.payments where invoice_id=(select id from t_invoice) and amount=30000 limit 1)$$,
  $$values(1::bigint)$$,
...
select results_eq(
  $$select status from public.invoices where id=(select id from t_invoice)$$,
  $$values('paid'::text)$$,
  'Cumulative completed payments settle the invoice only at full payment'
);
select throws_ok(
  $$select public.submit_invoice_payment((select id from t_invoice),1,'mpesa','AFTER-PAID')$$,
  NULL,'Already-paid invoice rejects another payment'
);

-- -----------------------------------------------------------------------------
-- RLS isolation: notifications, commissions and payouts
-- -----------------------------------------------------------------------------
reset role;
insert into public.notifications(user_id,title,content)
select (select id from t_ids where name='client_a'),'Isolation A','A only';
insert into public.notifications(user_id,title,content)
select (select id from t_ids where name='client_b'),'Isolation B','B only';
...
select (select id from t_ids where name='connector_b'),p.id,1000,20,200,'pending' from public.projects p where p.title='Release Test Project';
insert into public.payouts(recipient_id,recipient_role,project_id,amount,status)
select (select id from t_ids where name='connector_a'),'connector',p.id,4000,'pending' from public.projects p where p.title='Release Test Project';
insert into public.payouts(recipient_id,recipient_role,project_id,amount,status)
select (select id from t_ids where name='connector_b'),'connector',p.id,2000,'pending' from public.projects p where p.title='Release Test Project';

select set_config('request.jwt.claims',jsonb_build_object('sub',(select id from t_ids where name='connector_a')::text,'role','authenticated')::text,true);
set local role authenticated;
select results_eq($$select count(*)::bigint from public.commissions$$,$$values(1::bigint)$$,'Connector A cannot read Connector B commission');
select results_eq($$select count(*)::bigint from public.payouts$$,$$values(1::bigint)$$,'Connector A sees only Connector A payout');

-- Maintenance settlement protection is asserted against the final function body and schema.
...
select ok(
  pg_get_functiondef('public.sync_maintenance_subscription_after_payment()'::regprocedure) ilike '%status=''past_due''%',
  'Maintenance partial-payment protection remains in final function'
);
