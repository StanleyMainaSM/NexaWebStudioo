...
union all select id,'owner' from t_ids where name='owner'
...
select is(public.verify_portal_access_password('owner','Owner-Stage3-Reset-123!'),true,'Authorized Owner can unlock the Owner portal with the configured password');
select is(public.has_portal_access('owner'),true,'Owner unlock is active before the password reset');

select set_config('request.jwt.claims',jsonb_build_object('sub',(select id from t_ids where name='admin')::text,'role','authenticated','session_id',(select id::text from auth.sessions where user_id=(select id from t_ids where name='admin')) )::text,true);
select is(public.reset_portal_access_password('owner','Owner-Stage3-Reset-456!'),true,'Admin password reset succeeds after an existing Owner unlock');
...
select is(public.reset_portal_access_password('owner','Client-Stage3-Password-123!'),false,'Unauthorized Client cannot reset a portal password');
select is(public.change_portal_access_password('owner','Owner-Stage3-Reset-456!','Client-Stage3-Changed-123!'),false,'Unauthorized Client cannot change a portal password');
select throws_ok(
...
select is((select count(*) from private.portal_access_passwords where password_hash in ('Owner-Stage3-Password-123!','Owner-Stage3-Changed-123!','Owner-Stage3-Reset-123!','Owner-Stage3-Reset-456!')),0::bigint,'Management never stores plaintext passwords');
