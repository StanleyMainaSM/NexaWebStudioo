...
assert.match(themeSource, /DEFAULT_THEME:\s*Theme\s*=\s*'dark'/, 'Dark must be the default theme when no preference is saved.');
...
assert.match(themeSource, /isTheme\(stored\) \? stored : DEFAULT_THEME/, 'Only an explicit saved Light or Dark preference may override the default.');
...
assert.match(htmlSource, /const theme = stored === 'dark' \|\| stored === 'light' \? stored : 'dark'/, 'The pre-React default must be Dark.');
assert.match(htmlSource, /document\.documentElement\.dataset\.theme = theme/, 'The pre-React bootstrap must apply the resolved theme before React renders.');
assert.match(htmlSource, /document\.documentElement\.dataset\.theme = 'dark'/, 'Bootstrap failure must still fall back to Dark.');
assert.match(htmlSource, /theme === 'light' \? '#f6f7fb' : '#09090b'/, 'Initial theme-color must remain theme-aware with Dark as the fallback.');
