...
const projectSecurity = fs.readFileSync('supabase/migrations/20260831102000_website_creation_project_security.sql', 'utf8');
const previewSecurity = fs.readFileSync('supabase/migrations/20260831104000_website_creation_preview_and_attribution_hardening.sql', 'utf8');
...
assert.match(projectSecurity, /Creation project access fields are protected/);
assert.match(projectSecurity, /assign_creation_project_operator/);
assert.match(previewSecurity, /public_preview_token UUID UNIQUE/);
assert.match(previewSecurity, /preview_enabled BOOLEAN NOT NULL DEFAULT false/);
assert.match(previewSecurity, /Creation preview controls are protected/);
assert.match(previewSecurity, /Made with Avelixa/);
assert.match(previewSecurity, /get_public_creation_preview/);
assert.match(previewSecurity, /preview_enabled = true/);
