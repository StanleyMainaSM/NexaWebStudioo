...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      /*
       * Build the Owner dashboard user list.
       */
      const users =
        authUsers.map(
          (authUser) => {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
       ...
       *
       * No plaintext password is created or stored.
       */
      const {
        data: invitedUser,
        error: inviteError,
      ...
      /*
       * Roll back a newly created Auth account
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      /*
       * Make sure target Auth user exists.
       */
      const {
        data: targetUser,
        error:
      ...
      /*
       * Connector profile is required when adding
       * the Connector role.
       */
      if (role === "connector") {
        const {
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      /*
       * If Connector role was removed, deactivate the
       * connector profile rather than deleting it.
       *
       * This preserves historical commission/lead data.
       */
      if (
        selectedRole ===
        "connector"
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      /*
       * Never allow Owner self-deletion.
       */
      if (
        targetUserId ===
        ownerUser.id
      ...
      /*
       * Load target user first so the audit record can
      ...
      /*
       * Never allow deletion of another Owner.
       */
      const {
       ...
       *
       * Database records that reference the Auth user
       * should follow the existing database foreign-key
       * policies.
       */
      const {
        error: deleteError,
